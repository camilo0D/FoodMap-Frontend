...
const getMockMenu = (categoryName: string) => {
  const cat = (categoryName || "").toLowerCase();
  if (cat.includes("pizza")) {
    return [
      { name: "Pizza Marinera", description: "Masa artesanal con camarones, calamares y salsa de la casa", price: 28000, label: "Recomendado" },
      { name: "Pizza de Mariscos", description: "Queso mozzarella premium, pulpo, camarón y ajo rostizado", price: 32000, label: "Popular" },
      { name: "Pizza Suprema de Pollo", description: "Champiñones, pollo desmechado, pimentón y salsa blanca", price: 24000 },
      { name: "Pizza Margarita Especial", description: "Tomates cherry de la región, albahaca fresca y aceite de oliva", price: 18000 }
    ];
  } else if (cat.includes("marisco") || cat.includes("ceviche") || cat.includes("pescado") || cat.includes("pacífico") || cat.includes("tradicional")) {
    return [
      { name: "Cazuela de Mariscos", description: "Clásica cazuela del Pacífico con camarón, calamar, pulpo y almeja en leche de coco", price: 35000, label: "Especialidad" },
      { name: "Ceviche de Camarón", description: "Camarones frescos marinados en limón, cebolla morada y cilantro picado", price: 22000, label: "Popular" },
      { name: "Arroz con Coco y Camarón", description: "Delicioso arroz con coco frito acompañado de camarones salteados", price: 28000 },
      { name: "Pescado Frito Sabrozura", description: "Pargo rojo entero frito, acompañado de patacones y ensalada fresca", price: 30000 }
    ];
  } else if (cat.includes("hamburguesa") || cat.includes("rápida")) {
    return [
      { name: "Hamburguesa del Pacífico", description: "Carne de res premium 150g, camarones al ajillo encocados y queso fundido", price: 25000, label: "Especialidad" },
      { name: "Hamburguesa Doble Carne", description: "Doble carne de 120g, tocino ahumado, queso cheddar y salsa especial", price: 22000 },
      { name: "Papas Rústicas Gratinadas", description: "Papas rústicas doradas cubiertas con queso fundido y tocineta", price: 12000 },
      { name: "Salchipapa Criolla", description: "Papa criolla, salchicha premium, queso costeño rallado y salsas", price: 15000 }
    ];
  } else if (cat.includes("café") || cat.includes("cafe")) {
    return [
      { name: "Café Filtrado Especial", description: "Café de origen preparado en método artesanal V60", price: 6000 },
      { name: "Cappuccino de la Casa", description: "Espresso doble con leche vaporizada y un toque de canela", price: 7500 },
      { name: "Torta de Chontaduro", description: "Porción de torta artesanal a base de chontaduro y miel", price: 8000, label: "Recomendado" },
      { name: "Pan de Bono Recién Horneado", description: "Delicioso pan de bono relleno de queso caliente", price: 3000 }
    ];
  } else {
    return [
      { name: "Bandeja Paisa Criolla", description: "Frijoles, arroz, carne molida, chicharrón, huevo, chorizo y arepa", price: 26000, label: "Popular" },
      { name: "Sancocho de Gallina", description: "Sopa tradicional con gallina criolla, papa, yuca, plátano y arroz", price: 24000 },
      { name: "Chuleta de Cerdo Apanada", description: "Chuleta de cerdo crujiente con papas fritas y ensalada", price: 20000 },
      { name: "Empanadas de Pescado (3 unidades)", description: "Empanadas crujientes rellenas de guiso de pescado del Pacífico", price: 7000 }
    ];
  }
};
...
const ALL_MOCK_REVIEWS: Review[] = [
  { user: "Carlos M.", date: "12 may. 2026", rating: 5, text: "¡Increíble experiencia! El servicio fue excelente y la comida deliciosa. Sin duda el mejor lugar de la ciudad." },
  { user: "Valentina P.", date: "5 may. 2026", rating: 4, text: "Muy buen ambiente y sazón auténtico. Los precios son justos para la calidad que ofrecen." },
  { user: "Andrés R.", date: "28 abr. 2026", rating: 5, text: "Fui con toda la familia y quedamos encantados. El pescado fresco y la atención de primera." },
  { user: "María F.", date: "20 abr. 2026", rating: 3, text: "La comida estuvo bien, aunque tardó un poco en salir. El lugar es bonito y limpio." },
];

const getMockReviews = (_id: string): Review[] => {
  // Return a consistent subset of reviews for any restaurant
  return ALL_MOCK_REVIEWS.slice(0, 3);
};
...
const RestaurantDetailPage = () => {
  ...
  const menuItems = getMockMenu(categoryName);
  const defaultReviews = getMockReviews(rest.id);
  const allReviews = [...customReviews, ...defaultReviews];

  // Calculate review score distribution for UI decoration
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container max-w-4xl py-6 px-4 space-y-8 animate-fade-in">
        ...
        {/* Subtarea: Galería de imágenes (embla-carousel-react) */}
        <div className="relative group rounded-2xl overflow-hidden shadow-lg border border-border/40">
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
            {images.map((_, i) => (
              <span key={i} className="w-2.5 h-2.5 rounded-full bg-white/40 border border-white/20 transition-all duration-300" />
            ))}
          </div>
        </div>
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2 bg-muted/30 border border-border/40 px-4 py-2.5 rounded-xl self-start shrink-0">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] text-muted-foreground leading-none mt-1">{totalReviews} opiniones</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 border-t border-border/50 mt-6 pt-6 text-sm">
            ...
            {rest.telefono && (
              <div className="flex items-center gap-3 text-muted-foreground hover:text-foreground transition-colors">
                ...
                <a href={`tel:${rest.telefono}`} className="font-medium hover:underline hover:text-primary transition-all">
                  {rest.telefono}
                </a>
              </div>
            )}
        ...
        <section className="bg-card rounded-2xl border border-border/60 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-border/50 flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground font-semibold uppercase tracking-wider bg-muted/65 px-2.5 py-1 rounded-md">
              Precios Netos
            </span>
          </div>
                  ...
                  <tr key={i} className="hover:bg-muted/10 transition-colors group">
                    <td className="py-4 px-6 font-semibold text-foreground align-top">
                      <div className="flex items-center gap-2">
                        {item.name}
                        {item.label && (
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-primary/10 text-primary border border-primary/20 scale-95 uppercase tracking-wider shrink-0">
                            {item.label}
                          </span>
                        )}
                      </div>
                      <span className="block text-xs text-muted-foreground mt-1 sm:hidden">
                        {item.description}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-muted-foreground max-w-md hidden sm:table-cell align-top">
                      {item.description}
                    </td>
        ...
        {/* Subtarea: Calificación y Reseñas */}
          ...
          {loggedIn ? (
            ...
            >
              ...
              <div className="space-y-1">
                ...
                />
                {errors.comment && (
                  <p className="text-xs text-destructive font-medium mt-1">{errors.comment.message}</p>
                )}
              </div>
